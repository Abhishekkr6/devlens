        ...
        const unsubscribe = subscribeWS((event: any) => {
            ...
            // Handle org:removed events
                ...
                if (updatedUser?.orgIds && updatedUser.orgIds.length > 0) {
                    ...
                    const { activeOrgSlug: newActiveOrgSlug } = useUserStore.getState();
                    if (newActiveOrgSlug) {
                        console.log("[GlobalNotificationListener] Switching to org:", newActiveOrgSlug);
                        router.push(`/organization/${newActiveOrgSlug}/repos`);
                    }
                } else {
